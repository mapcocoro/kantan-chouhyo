...
### データ保存について
- **ローカル保存**: 入力内容はブラウザのLocalStorageに自動保存（サーバーには送信されません）
- **自社情報の保持**: 発行者情報は帳票タイプを変更しても保持されます
- **取引先情報の管理**: 取引先情報も自動保存されます（「取引先情報を保存しない」オプションあり）
- **共有PC利用時**: プライバシー保護のため、利用後は「取引先をクリア」で情報を削除してください
...
### 発注書の特殊機能
- **役割の自動切替**: 発注書選択時は自社情報が「受注者」欄に自動セット（発注者として発行するため）
- 納期条件（明細ごと/日付指定/期間指定）
